...
# 👤 MP Names Extractor
# ----------------------------------------
TITLE_TOKENS = r"(Shri|Smt\.?|Dr\.?|Prof\.?|Kumari|Km\.?|Ms\.?|Miss)"
...
def extract_mp_names(full_text: str):
    ...
    m = re.search(
        rf"[†*]?\s*\d{{1,4}}[^:\n]{{0,80}}?(?P<names>(?:\s+{TITLE_TOKENS}[^:{{0,120}}]?)+?)\s*:\s*Will the Minister",
        text,
        flags=re.I | re.S
    )
    ...
    if m:
        ...
        wm = re.search(r"Will the Minister", text, re.I)
        if wm:
            pre = text[:wm.start()]
            colon = pre.rfind(":")
            if colon != -1:
                block = pre[max(0, colon - 220):colon]

    if not block:
        return []

    block = re.sub(r"^[^A-Za-z]*(?:\d{1,4}\.?)?\s*", "", block)
    ...
    found = []
    for title, name in name_pat.findall(block):
        nm = f"{title} {name}".strip()
        nm = re.sub(r"[\s,;]+$", "", nm)
        found.append(nm)
...
def extract_details_from_blob(blob_data: bytes):
    ...
    for line in lines:
        u = line.upper()
        if "LOK SABHA" in u:
            details["house"] = "Lok Sabha"; break
        if "RAJYA SABHA" in u:
            details["house"] = "Rajya Sabha"; break
            ...
            if re.search(r"(?:STARRED|UNSTARRED)?\s*QUESTION\s*NO\.?", line, re.I):
                ...
                while k < len(lines):
                    if "ANSWERED ON" in lines[k].upper():
                        kk = k + 1
                        while kk < len(lines) and (DATE_RE.search(lines[kk]) or "ANSWERED ON" in lines[kk].upper() or re.fullmatch(r"[-–—:]+", lines[kk])):
                            kk += 1
                        if kk < len(lines):
